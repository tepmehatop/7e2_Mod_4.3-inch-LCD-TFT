...
void Feed_Ext_Left()
{
   ...
   {  
      if (cycle_flag == false && Pass_Nr <= Pass_Total)
      {
         ...
         long Infeed_Value = (int)((float)MOTOR_X_STEP_PER_REV * (Ap / 2) / SCREW_X +0.5) * McSTEP_X;
         if (Infeed_Value == 0)
         {
...
void Feed_Ext_Front()
{  
  if (TENSION_Z > 0 || OTSKOK_Z < REBOUND_Z){ ///если натяг больше нуля или если отскок отключен
  if ((Motor_X_Pos == Limit_Pos_Rear && Step == 0 && Motor_Z_Pos == Limit_Pos_Right && Step_X_flag == OFF) ||
      (Motor_X_Pos == Limit_Pos_Rear && Step == 0 && Motor_Z_Pos == Null_Z_Pos && Step_X_flag == OFF))
   {  
      if (cycle_flag == false && Pass_Nr <= Pass_Total)
      {
         cycle_flag = true;
         ...
         if  (OTSKOK_Z < REBOUND_Z){ ///////если отскок отключен
          
         if (Infeed_Value == 0)
         {
            if (Pass_Nr == 1) Limit_Pos_Left = (Null_Z_Pos + 1);
            else              Limit_Pos_Left = (Limit_Pos_Right + TENSION_Z);
         }
         else
         {
            if (Pass_Nr == 1) Limit_Pos_Left = (Null_Z_Pos + 1);
            else              Limit_Pos_Left = (Limit_Pos_Right + Infeed_Value + TENSION_Z);
         }
         }
         else {
         if (Infeed_Value == 0)
         {
            if (Pass_Nr == 1) Limit_Pos_Left = (Null_Z_Pos + 1);
            else              Limit_Pos_Left = (Limit_Pos_Right + OTSKOK_Z);
         }
         else
         {
            if (Pass_Nr == 1) Limit_Pos_Left = (Null_Z_Pos + 1);
            else              Limit_Pos_Left = (Limit_Pos_Right + OTSKOK_Z + Infeed_Value);
         }
         }
         Limit_Left_LED_On();
         BeepBeep();
      ...
      else if (cycle_flag == false && Pass_Nr > Pass_Total)
      {
         ...
         a_flag = false;
         b_flag = false; 
         Limit_Pos_Left = Limit_Pos_Right + TENSION_Z;
         Feed_Left(a_flag, b_flag);
         }
         else {
         a_flag = false;
         b_flag = false; 
         Limit_Pos_Left = Limit_Pos_Right + OTSKOK_Z;
         Feed_Left(a_flag, b_flag);
         }
         Limit_Left_LED_Off();
         Limit_Right_LED_Off();
   ...
   else if (Motor_X_Pos == Limit_Pos_Rear && Motor_Z_Pos == Limit_Pos_Left && Step_X_flag == OFF)
   {
      ...
      Limit_Pos_Right = (Limit_Pos_Left - TENSION_Z);
      Feed_Right(a_flag, b_flag);
      Step = 1;
   }
   else if (Motor_X_Pos == Limit_Pos_Rear && Step == 1 && Motor_Z_Pos == Limit_Pos_Right && Step_Z_flag == OFF)
   {
      if (Pass_Nr <= Pass_Total)
      {
         a_flag = false;
   ...
   else if (Motor_X_Pos == Limit_Pos_Front && Step == 2 && Motor_Z_Pos == Limit_Pos_Right && Step_X_flag == OFF)
   {
      cycle_flag = false;
      Step = 3;
      if  (OTSKOK_Z < REBOUND_Z){ ///////если отскок отключен
      ...
      a_flag = false;
      b_flag = false;
      Limit_Pos_Right = (Limit_Pos_Left - OTSKOK_Z);
      Limit_Right_LED_On();
      Feed_Right(a_flag, b_flag);
      } 
   }

   else if (Motor_X_Pos == Limit_Pos_Front && Step == 3 && Motor_Z_Pos == Limit_Pos_Right && Step_Z_flag == OFF)
   {  
      a_flag = false;
      b_flag = false;
