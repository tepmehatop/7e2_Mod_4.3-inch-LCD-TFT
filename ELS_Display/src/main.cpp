...
void loop()
{
    ...
    // LVGL tick: реальная дельта времени, а не фиксированный шаг.
    // Причина: если loop() занимает 35мс (5мс delay + 30мс рендер),
    // но lv_tick_inc(5) → LVGL думает прошло 5мс → touch читается каждые
    // 210мс вместо 30мс → двойной тап занимает 2-3 секунды.
